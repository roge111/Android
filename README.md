...
6) `onRestart` — вызывается перед тем, как Activity вновь будет видна.
7) `onDestroy` — вызывается перед тем, как Activity будет уничтожена. Освобождаются ресурсы и т. д.
...
Смена конфигурации Android - это изменение настроек устройства: ориентация экрана, язык, размер экрана и тд. Например, повоторо экрана - в таком случае приложения пересоздает Activity для применения новых настроек. Если мы хотим сохранить данные при пересоздании, то важно изучить, что такое класс `Bundle`
